...
export async function GET(context: APIContext) {
  ...
  return rss({
    title: 'S3r4ph1el',
    description: 'Scripta · Opera · Acta — Offensive Security by Enzo Teles.',
    site: context.site!,
    items: all.map((entry) => ({
      title: entry.data.title,
      description: entry.data.description ?? '',
      pubDate: entry.data.date,
      link: `/en/${urlPrefix[entry.collection as Section]}/${entry.slug}/`,
    })),
